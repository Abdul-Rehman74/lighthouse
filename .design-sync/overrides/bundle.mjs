// forked from design-sync lib/bundle.mjs — Next.js shims: replace next/link, next/image,
// next/navigation, next/font with simple React equivalents so the bundle doesn't pull in
// the full Next.js runtime (which uses process, Buffer, etc. unavailable in the browser).
// Also injects a process shim for any residual process references in bundled deps.
...
// Shim next/* imports to simple browser-safe React equivalents so the bundle
// doesn't pull in Node.js-only runtime code (process, Buffer, etc.).
export const nextShim = {
  name: 'next-shim',
  setup(b) {
    b.onResolve({ filter: /^next\// }, (args) => ({ path: args.path, namespace: 'next-shim' }));
    const linkShim = `
Object.defineProperty(exports,"__esModule",{value:true});
var R=window.React;
exports.default=R.forwardRef(function Link(p,ref){
  var href=p.href,children=p.children,rest=Object.assign({},p);
  delete rest.href;delete rest.children;delete rest.prefetch;delete rest.replace;delete rest.scroll;delete rest.shallow;delete rest.locale;delete rest.legacyBehavior;delete rest.passHref;
  return R.createElement('a',Object.assign({href:typeof href==='string'?href:(href&&href.pathname)||'#'},rest,{ref:ref}),children);
});`;
    const imageShim = `
Object.defineProperty(exports,"__esModule",{value:true});
var R=window.React;
exports.default=R.forwardRef(function Image(p,ref){
  var src=p.src,rest=Object.assign({},p);
  delete rest.placeholder;delete rest.blurDataURL;delete rest.loader;delete rest.quality;delete rest.priority;delete rest.loading;delete rest.sizes;delete rest.fill;delete rest.unoptimized;
  return R.createElement('img',Object.assign({},rest,{src:typeof src==='string'?src:(src&&src.src)||'',ref:ref}));
});`;
    const fontShim = `exports.Inter=exports.Fraunces=exports.Nunito=exports.Caveat=exports.Lato=exports.Poppins=exports.Roboto=exports.OpenSans=exports.Montserrat=function(){return{className:'',style:{}}};`;
    const navShim = `
exports.useRouter=function(){return{push:function(){},replace:function(){},back:function(){},forward:function(){},refresh:function(){},prefetch:function(){},pathname:'/',query:{}}};
...
exports.redirect=function(){};`;
    const serverShim = `exports.cookies=function(){return{get:function(){return undefined;},set:function(){},has:function(){return false;}}};exports.headers=function(){return new Map();};`;
    b.onLoad({ filter: /^next\/link$/, namespace: 'next-shim' }, () => ({ contents: linkShim, loader: 'js' }));
    b.onLoad({ filter: /^next\/image$/, namespace: 'next-shim' }, () => ({ contents: imageShim, loader: 'js' }));
    b.onLoad({ filter: /^next\/font\/google$/, namespace: 'next-shim' }, () => ({ contents: fontShim, loader: 'js' }));
    b.onLoad({ filter: /^next\/font\/local$/, namespace: 'next-shim' }, () => ({ contents: fontShim, loader: 'js' }));
    b.onLoad({ filter: /^next\/navigation$/, namespace: 'next-shim' }, () => ({ contents: navShim, loader: 'js' }));
    b.onLoad({ filter: /^next\/server$/, namespace: 'next-shim' }, () => ({ contents: serverShim, loader: 'js' }));
    b.onLoad({ filter: /^next\//, namespace: 'next-shim' }, () => ({ contents: 'module.exports={};', loader: 'js' }));
  },
};

// process shim: any residual process.* after define substitution (e.g. process.nextTick in
// bundled deps that esbuild couldn't statically replace).
export const processShim = {
  name: 'process-shim',
  setup(b) {
    b.onResolve({ filter: /^process$/ }, () => ({ path: 'process-shim', namespace: 'shim' }));
    b.onLoad({ filter: /^process-shim$/, namespace: 'shim' }, () => ({
      contents: 'module.exports={env:{NODE_ENV:"production",NEXT_RUNTIME:"browser"},browser:true,nextTick:function(fn,a,b,c){return setTimeout(function(){fn(a,b,c)},0)},version:"",versions:{}};',
      loader: 'js',
    }));
  },
};
...
function sharedBuildOptions({ nodePaths, tsconfig }) {
  ...
  const plugins = [nextShim, processShim, reactShim];
